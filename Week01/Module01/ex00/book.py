...
class Book:
    def __init__(self, name, last_update, creation_date, recipes_list):
        if not name or not isinstance(name, str):
            ...
            exit()
        elif not last_update or not isinstance(last_update, datetime.datetime):
            print("Please Enter a datetime for last_update")
            exit()
        elif not creation_date or not isinstance(creation_date, datetime.datetime):
            print("Please Enter a datetime for creation_date")
            exit()
        elif not recipes_list or not isinstance(recipes_list, list):
    ...
    def get_recipes_by_types(self, recipe_type):
        ...
        if recipe_type and isinstance(recipe_type, str) and recipe_type in ["starter", "lunch", "dessert"]:
            for recipe in self.recipes_list:
